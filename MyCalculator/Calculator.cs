...
namespace MyCalculator {
    public class Calculator {
        ...
        public string PressSqrt(string textOnScreen)
        {
            double firstDigit = double.Parse(textOnScreen);
            double sqrt = Math.Sqrt(firstDigit);
            return sqrt.ToString();
        }

        public string PressMinus(string textOnScreen)
        {
            _firstDigit = double.Parse(textOnScreen);
            _currentOperation = "minus";
            return "0";
        }
        public string PressMulti(string textOnScreen)
        {
            _firstDigit = double.Parse(textOnScreen);
            _currentOperation = "multiplication";
            return "0";
        }
        public string PressPrecent(string textOnScreen)
        {
            _firstDigit = double.Parse(textOnScreen);
            _currentOperation = "precent";
            return "0";
        }
        public string PressDivision(string textOnScreen)
        {
            _firstDigit = double.Parse(textOnScreen);
        ...
        public string PressEqualButton(string textOnScreen) {
            ...
            switch (_currentOperation) {
                ...
                case "division":
                    ...
                    break;
                case "precent":
                    _result = _firstDigit / 100 * secondDigit ;
                    break;
                default:
                    break;
            }
